...
def editArticles(id):
    ...
    if form.validate_on_submit():
        ...
        db.session.add(article)
        db.session.commit()
        flash(u'article updated！', 'success')
...
def delete_article():
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            db.session.rollback()
            flash(u'deleting failed！', 'danger')
        else:
            flash(u'successfully deleted%scomments！' % count, 'success')
    if form.errors:
        flash(u'failed to delete！', 'danger')
...
def delete_articles():
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            flash(u'successfully deleted%sarticles and%scomments！' % (len(articleIds), count), 'success')
    if form.errors:
        flash(u'failed to delete！', 'danger')
...
def disable_comment(id):
    ...
    db.session.commit()
    flash(u'hiding successfully！', 'success')
...
def enable_comment(id):
    ...
    db.session.add(comment)
    db.session.commit()
    flash(u'successfully changed to visable！', 'success')
...
def delete_comment(id):
    ...
    try:
        ...
        flash(u'successfully deleted！', 'success')
...
def manage_comments():
    ...
    if form.errors:
        flash(u'failed to submit ,check again !', 'danger')
        return redirect(url_for('.manage_comments'))
...
def manage_articleTypes():
    ...
    if form.validate_on_submit():
        ...
        if articleType:
            flash(u'already existed。', 'danger')
        else:
            ...
            db.session.commit()
            flash(u'type addedd successfully ！', 'success')
        return redirect(url_for('.manage_articleTypes'))
    if form.errors:
        flash(u'failed to add,plese check。', 'danger')
        return redirect(url_for('.manage_articleTypes'))
    ...
    if form2.validate_on_submit():
        ...
        if articleType.is_protected:
            if form2.name.data != articleType.name or \
                ...
                flash(u'you can only change the default types and the navigation', 'danger')
            else:
                ...
                db.session.add(articleType)
                db.session.commit()
                flash(u'default type changed successfully！', 'success')
            ...
            db.session.add(articleType)
            db.session.commit()
            flash(u'successfully changed', 'success')
        return redirect(url_for('.manage_articleTypes', page=page))
    if form2.errors:
        flash(u'failed to change ,please check', 'danger')
        return redirect(url_for('.manage_articleTypes', page=page))
...
def delete_articleType(id):
    ...
    if articleType.is_protected:
        flash(u'attention:you dont have permission to change！', 'danger')
    ...
    try:
        ...
        flash(u'successfull !%sarticles addedd to <default type>。' % count, 'success')
    return redirect(url_for('admin.manage_articleTypes', page=page))
...
def manage_articleTypes_nav():
    ...
    if form.validate_on_submit():
        ...
        if menu:
            ...
            db.session.commit()
            page = -1
            flash(u'successfully added the menu！', 'success')
    ...
    if form2.validate_on_submit():
        ...
        if Menu.query.filter_by(name=name).first() \
            and Menu.query.filter_by(name=name).first().id != nav_id:
                flash(u'failed to change ,already existed', 'danger')
        else:
            nav = Menu.query.get_or_404(nav_id)
            nav.name = name
            db.session.add(nav)
            db.session.commit()
            flash(u'successfullly changed the nav！', 'success')
        return redirect(url_for('admin.manage_articleTypes_nav', page=page))
    if form2.errors:
        flash(u'failed ,please check', 'danger')
        return redirect(url_for('admin.manage_articleTypes_nav', page=page))
...
def delete_nav(id):
    ...
    try:
        ...
        db.session.rollback()
        flash(u'failed to delete the navigation！', 'danger')
    else:
        flash(u'successfully deleted！%s' % count, 'success')
    return redirect(url_for('admin.manage_articleTypes_nav', page=page))
...
def nav_sort_up(id):
    ...
    if pre_menu:
        ...
        db.session.commit()
        flash(u'successfully uprised！', 'success')
    else:
        flash(u'make it top ！', 'danger')
    return redirect(url_for('admin.manage_articleTypes_nav', page=page))
...
def nav_sort_down(id):
    ...
    if latter_menu:
        ...
        db.session.commit()
        flash(u'successfully setted', 'success')
    else:
        flash(u'leave it on the buttom！', 'danger')
    return redirect(url_for('admin.manage_articleTypes_nav', page=page))
...
def custom_blog_info():
    ...
    if form.validate_on_submit():
        ...
        flash(u'blog info changed successfully！', 'success')
        return redirect(url_for('admin.custom_blog_info'))
...
def delete_plugin(id):
    ...
    try:
        ...
        db.session.rollback()
        flash(u'failed to delete plugin！', 'danger')
    else:
        flash(u'successfully deleted plugin！' ,'success')
...
def plugin_sort_up(id):
    ...
    if pre_plugin:
        ...
        db.session.commit()
        flash(u'plugin uprised！', 'success')
    else:
        flash(u'plugin already on the top！', 'danger')
    return redirect(url_for('admin.custom_blog_plugin', page=page))
...
def plugin_sort_down(id):
    ...
    if latter_plugin:
        ...
        db.session.commit()
        flash(u'plugin setted down！', 'success')
    else:
        flash(u'plugin alredy on the buttom！', 'danger')
    return redirect(url_for('admin.custom_blog_plugin', page=page))
...
def edit_plugin(id):
    ...
    if form.validate_on_submit():
        ...
        if plugin_check and plugin_check.id != id:
            ...
            db.session.add(plugin)
            db.session.commit()
            flash(u'changed successfully', 'success')
        return redirect(url_for('admin.custom_blog_plugin', page=page))
...
def change_password():
    ...
    if form.validate_on_submit():
        if current_user.verify_password(form.old_password.data):
            ...
            flash(u'wrong password,try again！', 'danger')
            return redirect(url_for('admin.account'))
...
def edit_user_info():
    ...
    if form2.validate_on_submit():
        if current_user.verify_password(form2.password.data):
            ...
            db.session.add(current_user)
            db.session.commit()
            flash(u'user info changed！', 'success')
            return redirect(url_for('admin.account'))
        else:
            flash(u'wrong password,try again！', 'danger')
            return redirect(url_for('admin.account'))
